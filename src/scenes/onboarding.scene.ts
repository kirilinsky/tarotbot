...
const FOCUS_AREAS = ["Любовь", "Работа", "Саморазвитие", "Финансы", "Семья", "Здоровье", "Другое"];
const LIFE_PHASES = ["В поиске себя", "Переживаю трудности", "Всё стабильно", "Готов(а) к переменам", "Влюблён(а)", "Неопределённость"];
...
const GENDER_MAP: Record<string, string> = { Мужской: "male", Женский: "female" };
const AGE_MAP: Record<string, string> = { "до 18": "under_18", "18–25": "18_25", "26–35": "26_35", "36–45": "36_45", "46–60": "46_60", "60+": "60_plus" };
const FOCUS_MAP: Record<string, string> = { Любовь: "love", Работа: "career", Саморазвитие: "self", Финансы: "money", Семья: "family", Здоровье: "health", Другое: "other" };
const LIFE_PHASE_MAP: Record<string, string> = { "В поиске себя": "searching", "Переживаю трудности": "crisis", "Всё стабильно": "stable", "Готов(а) к переменам": "transition", "Влюблён(а)": "in_love", Неопределённость: "uncertain" };
const RELATIONSHIP_MAP: Record<string, string> = { Сингл: "single", "В отношениях": "in_relationship", "Всё сложно": "complicated" };
...
export const onboardingWizard = new Scenes.WizardScene<BotContext>(
  ...
  // Step 1 — age group
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !GENDERS.includes(ctx.message.text)) {
  ...
  // Step 2 — focus area
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !AGE_GROUPS.includes(ctx.message.text)) {
      await ctx.reply("Пожалуйста, выбери вариант из кнопок 👇");
      return;
    }
  ...
  // Step 3 — life phase
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !FOCUS_AREAS.includes(ctx.message.text)) {
      await ctx.reply("Пожалуйста, выбери вариант из кнопок 👇");
      return;
    }
  ...
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !LIFE_PHASES.includes(ctx.message.text)) {
      await ctx.reply("Пожалуйста, выбери вариант из кнопок 👇");
      return;
    }
  ...
  // Step 5 — zodiac sign
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !RELATIONSHIP_STATUSES.includes(ctx.message.text)) {
      await ctx.reply("Пожалуйста, выбери вариант из кнопок 👇");
      return;
    }
    (ctx.wizard.state as State).relationship_status = RELATIONSHIP_MAP[ctx.message.text];
  ...
  // Step 6 — save to DB
  async (ctx) => {
    if (!ctx.message || !("text" in ctx.message) || !ZODIAC_SIGNS.includes(ctx.message.text)) {
      await ctx.reply("Пожалуйста, выбери вариант из кнопок 👇");
      return;
    }
